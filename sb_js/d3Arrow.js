(function (global, factory) {
    ...
    class Arrow {
      ...
      createArrowHead(svg, options) {
        ...
        marker.attr("id", Utils.uniqueClass("arrow-head", this.uid)).attr("viewBox", "0 0 10 10").attr("refX", 8).attr("refY", 5).attr("markerWidth", 20).attr("markerHeight", 50).attr("markerUnits", "userSpaceOnUse").attr("stroke", "none").attr("orient", "auto").attr("fill", options.color).append("path").attr("d", "M 0 0 L 10 5 L 0 10 z");
        return marker;
      }

      createPath(svg, options) {
        return svg.append("path").attr("stroke", options.color).attr("stroke-width", 5).attr("stroke-linecap", "round").attr("fill", "none");
      }
