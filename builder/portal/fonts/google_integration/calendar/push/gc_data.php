...
if(isset($_POST)){
	...
	foreach($fetchdata as $data){
		...
		$query = "INSERT INTO `googledrive_files` (name, size, type, token, file_id, modified_by) VALUES ('".$fileName."', '".$f_size."', '".$f_type."', '".$f_token."', '".$f_id."', '".$ui."')";
		
		$execQt = mysqli_query($theodore_con, $query);
		
		if(!$execQt){
			die(mysqli_error($theodore_con));
		} else {
