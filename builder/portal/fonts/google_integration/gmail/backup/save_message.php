...
$result = array();
    ...
    foreach($fetchdata as $msg){
        ...
        if(mysqli_num_rows($chkExist) == 0){
            $save = mysqli_query($theodore_con, "INSERT INTO `gmail_integration` (`client_id`, `user_id`, `gmail_profile_id`, `message_id`, `thread_id`, `raw_message_id`, `subject`, `to`, `from`, `reply_to`, `in_reply_to`, `date_utc`, `date_mc`, `body`, `snippet`, `msg_body_mime`, `sbdefined_snippet`, `moved_to_notes`, `moved_by`) VALUES ('".$dec_ci."', '".$dec_ui."', '" . $profile_id . "', '" . $msg_message_id . "', '" . $msg_thread_id . "', '".$msg_raw_message_id."',  '" . $msg_subject . "', '" . $msg_to . "', '" . $msg_from . "', '".$msg_reply_to."', '".$msg_in_reply_to."', '" . $msg_date . "', '" . $msg_microtime . "', '" . $msg_body . "', '" . $msg_snippet . "', '".$msg_body_mime."', '".$msg_sb_snippet."', 1, '".$dec_ui."')");
