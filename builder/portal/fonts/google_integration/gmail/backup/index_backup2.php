...
<script type="text/javascript">
    ...
    function listMessages(profile_id, maxresult) {
        ...
        gapi.client.gmail.users.messages.list({
            ...
            var cnt_state = 0;
            while(c < response.result.messages.length){
                ...
                gapi.client.gmail.users.messages.get({
                    ...
                    $.post("save.php?ui=" + GetQueryStringParams("ui") + "&ci=" + GetQueryStringParams("ci"), JSON.stringify({data: arr_msg}), function(response) {
                        ...
                        chkCounts(cnt_state, c, profile_id, maxresult);
    ...
    function chkCounts(cnt_state, c, profile_id, maxresult){
    
        if(cnt_state == maxresult){ //all items are existing
            console.log('state count ' + cnt_state);
            console.log('max result ' + maxresult);
            listMessages(profile_id, maxresult + 10);
        } else {
            
            if(c==cnt_state){
                console.log(cnt_state);
                console.log(c);
                console.log(maxresult);
                
                    setTimeout(function () {
                        pagereturn(profile_id, true);
                    }, 1000);        
            }
