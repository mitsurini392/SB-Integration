...
if($getUserInfo && mysqli_num_rows($getUserInfo) != 0){
    ...
    foreach($fetchdata as $arrdata){
        ...
        $sb_message_id = $arrdata['message_id'];
        $projectname = $arrdata['projectname'];
        $status = $arrdata['status'];
        $sbsnippet = $arrdata['sbsnippet'];
        $sbsnippet = mysqli_real_escape_string($theodore_con, trim($sbsnippet));
                ...
                extract($fetchEmailData);
                    ...
                    if(mysqli_query($theodore_con, $insert_main)){
                        ...
                        if(mysqli_query($theodore_con, $insert_body)){
                            ...
                            $update_moved = "UPDATE `gmail_integration` SET moved_to_notes = 1, sbdefined_snippet = '".$sbsnippet."' WHERE id = '".$sb_message_id."'";
                            
                            if(mysqli_query($theodore_con, $update_moved)){
                                $status = true;
                            } else {
