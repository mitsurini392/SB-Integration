...
require("class.phpmailer.php");
	...
	You can access the Small Builders through the following link:<br/><br/>
		...
		if(!$exec)
			...
			else
			{
				if($clienttype=="subcontractor")
					{
						$select_clientName = mysql_query("DELETE from `cs_userspecific_forms` where user_id= '".$id."'");
						$addUserSpecific = $as_create->add('cs_userspecific_forms', array("$id","7"));
						$addUserSpecific = $as_create->add('cs_userspecific_forms', array("$id","36"));
					}
					
				else if($clienttype=="supplier")
					{
						$select_clientName = mysql_query("DELETE from `cs_userspecific_forms` where user_id= '".$id."'");
						$addUserSpecific = $as_create->add('cs_userspecific_forms', array("$id","9"));
						$addUserSpecific = $as_create->add('cs_userspecific_forms', array("$id","37"));
					}
				else
					{
					...
					else
					{
						echo "<script type='text/javascript'> 
							alert('User Details was successfully updated.'); 
